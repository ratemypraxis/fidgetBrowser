  ...
  const handTrackingSketch = function(p) {
    ...
    function initializeHandpose() {
      ...
      try {
        ...
        console.error('model not loading', error);
      }
    }
    ...
    p.draw = function() {
      ...
      if (video && video.loadedmetadata) {
        ...
        p.textSize(14);
        p.text('asking for cam', p.width / 2, p.height / 2);
        return;
      } else {
        p.background(0);
        p.fill(255);
    ...
    function drawHands() {
      for (let i = 0; i < hands.length; i++) {
        ...
        if (landmarks.length >= 5) {
          //thumb base
          const [x1, y1] = landmarks[1]; 
          //thumb tip
          const [x4, y4] = landmarks[4]; 
    ...
    function drawNoHandsMessage() {
      ...
      p.textAlign(p.CENTER, p.CENTER);
      p.textSize(16);
      p.text('Cannot find your hand!', p.width / 2, p.height / 2);
      p.textSize(12);
      p.text('Show your hands please', p.width / 2, p.height / 2 + 25);
    }
